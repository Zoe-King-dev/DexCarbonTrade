...
const exchange_name = 'CarbonTradingPlatform';             // TODO: fill in the name of your exchange
const token_name = 'CarbonCoinToken';                // TODO: replace with name of your token
const token_symbol = 'CCT';              // TODO: replace with symbol for your token
...
async function removeAllLiquidity(maxSlippagePct) {
  ...
  amount = await exchange_contract.connect(signer).removeAllLiquidity(
...
async function getExchangeRates(maxSlippagePct){
  reserves = await exchange_contract.connect(signer).getLiquidity();
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  balance = await token_contract.connect(signer).balanceOf(defaultAccount);
  await token_contract.connect(signer).approve(exchange_address, balance);
  await exchange_contract.connect(signer).swapTokensForETH(
...
const sanityCheck = async function() {
  var swap_fee = await exchange_contract.connect(signer).getSwapFee();
  ...
  var start_tokens = await token_contract.connect(signer).balanceOf(defaultAccount);
  ...
  if (Number(swap_fee[0]) == 0) {
      ...
      var user_tokens1 = await token_contract.connect(signer).balanceOf(defaultAccount);
      ...
      var user_tokens2 = await token_contract.connect(signer).balanceOf(defaultAccount);
      ...
      defaultAccount = accounts[0];
      var state2 = await getPoolState();
      user_tokens2 = await token_contract.connect(signer).balanceOf(defaultAccount);
      ...
      // TODO: remove this line
      console.log(state3)
      var user_tokens3 = await token_contract.connect(signer).balanceOf(defaultAccount);
      ...
      console.log(state4)
      var user_tokens4 = await token_contract.connect(signer).balanceOf(defaultAccount);
      ...
      console.log(state5);
      var user_tokens5 = await token_contract.connect(signer).balanceOf(defaultAccount);
  ...
  // LP provider rewards implemented
      ...
      var user_tokens6 = await token_contract.connect(signer).balanceOf(defaultAccount);
      score += check("Test removing all liquidity", swap_fee[0], 

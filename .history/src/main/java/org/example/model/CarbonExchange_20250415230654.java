...
public class CarbonExchange {
    ...
    private String name = "Carbon Credit Exchange";
    ...
    @ElementCollection
    @CollectionTable(name = "lp_shares", joinColumns = @JoinColumn(name = "exchange_id"))
    @MapKeyColumn(name = "address")
    @Column(name = "shares")
    private Map<String, BigDecimal> lpShares = new HashMap<>();

    @Column(nullable = false)
    private BigDecimal carbonCreditFeeReserves = BigDecimal.ZERO;

    @Column(nullable = false)
    private BigDecimal baseCurrencyFeeReserves = BigDecimal.ZERO;

    @Column(nullable = false)
    private int swapFeeNumerator = 3;

    @Column(nullable = false)
    private int swapFeeDenominator = 100;

    @Column(nullable = false)
    private int exchangeRateMultiplier = 100000;
    ...
    public void createPool(BigDecimal amountCarbonCredits, BigDecimal amountBaseCurrency) {
        ...
        totalShares = BigDecimal.valueOf(100000);
    }
    ...
    public void addLiquidity(String provider, BigDecimal amountBaseCurrency, 
        ...
        lpShares.merge(provider, newShares, BigDecimal::add);
    }
    ...
    public void removeLiquidity(String provider, BigDecimal amountBaseCurrency,
        ...
        BigDecimal sharesToRemove = amountBaseCurrency.multiply(totalShares)
                .divide(baseCurrencyReserves, 18, RoundingMode.DOWN);
        
        if (lpShares.getOrDefault(provider, BigDecimal.ZERO).compareTo(sharesToRemove) < 0) {
        ...
        BigDecimal carbonCreditsToRemove = carbonCreditReserves.multiply(sharesToRemove)
                .divide(totalShares, 18, RoundingMode.DOWN);
        ...
        BigDecimal feeFraction = sharesToRemove.divide(totalShares, 18, RoundingMode.DOWN);
        BigDecimal carbonCreditFees = carbonCreditFeeReserves.multiply(feeFraction);
        BigDecimal baseCurrencyFees = baseCurrencyFeeReserves.multiply(feeFraction);
        ...
        k = carbonCreditReserves.multiply(baseCurrencyReserves);
        totalShares = totalShares.subtract(sharesToRemove);
        lpShares.put(provider, lpShares.get(provider).subtract(sharesToRemove));
    ...
    public void swapCarbonCreditsForBaseCurrency(String trader, BigDecimal amountCarbonCredits,
        ...
        BigDecimal fee = amountCarbonCredits.multiply(BigDecimal.valueOf(swapFeeNumerator))
                .divide(BigDecimal.valueOf(swapFeeDenominator), 18, RoundingMode.DOWN);
        ...
        carbonCreditFeeReserves = carbonCreditFeeReserves.add(fee);
    }
    ...
    public void swapBaseCurrencyForCarbonCredits(String trader, BigDecimal amountBaseCurrency,
        ...
        BigDecimal fee = amountBaseCurrency.multiply(BigDecimal.valueOf(swapFeeNumerator))
                .divide(BigDecimal.valueOf(swapFeeDenominator), 18, RoundingMode.DOWN);
        ...
        baseCurrencyFeeReserves = baseCurrencyFeeReserves.add(fee);
    ...
    private BigDecimal calculateExchangeRate() {
        return carbonCreditReserves.multiply(BigDecimal.valueOf(exchangeRateMultiplier))
                .divide(baseCurrencyReserves, 18, RoundingMode.DOWN);
    }

    public BigDecimal getLiquidity(String address) {
        return lpShares.getOrDefault(address, BigDecimal.ZERO);
    }
} 
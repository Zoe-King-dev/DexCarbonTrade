...
public class CarbonExchange {
    ...
    @ElementCollection
    @CollectionTable(name = "token_balances", joinColumns = @JoinColumn(name = "exchange_id"))
    @MapKeyColumn(name = "address")
    @Column(name = "balance")
    private Map<String, BigDecimal> baseBalances = new HashMap<>();
    ...
    public void prePersist() {
        ...
        if (this.liquidityProviders == null) {
            this.liquidityProviders = new HashMap<>();
        }
    }
    ...
    public BigDecimal getCarbonFeeReserves() {
        return carbonFeeReserves;
    }

    public void setCarbonFeeReserves(BigDecimal carbonFeeReserves) {
        this.carbonFeeReserves = carbonFeeReserves;
    }

    public BigDecimal getBaseFeeReserves() {
        return baseFeeReserves;
    }

    public void setBaseFeeReserves(BigDecimal baseFeeReserves) {
        this.baseFeeReserves = baseFeeReserves;
    }

    public BigDecimal getBaseBalance(String address) {
        return baseBalances.getOrDefault(address, BigDecimal.ZERO);
    }

    public void setBaseBalance(String address, BigDecimal amount) {
        baseBalances.put(address, amount);
    }
} 
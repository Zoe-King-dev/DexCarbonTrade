...
public class CarbonExchange {
    ...
    public void addLiquidity(String provider, BigDecimal amountBaseCurrency, 
        ...
        // 如果是空池，進行初始化
        if (getTotalCarbonReserves().equals(BigDecimal.ZERO) && 
            getTotalBaseReserves().equals(BigDecimal.ZERO)) {
            // 從用戶餘額中扣除 CCT
            token.transfer(provider, token.getAddress(), INITIAL_CCT_AMOUNT);
            
            // 設置初始儲備
            setTotalCarbonReserves(INITIAL_CCT_AMOUNT);
            setTotalBaseReserves(INITIAL_USDC_AMOUNT);
            
            // 計算並設置 k 值
            setK(INITIAL_CCT_AMOUNT.multiply(INITIAL_USDC_AMOUNT));
            
            // 分配初始 LP 份額
            setTotalShares(INITIAL_TOTAL_SHARES);
        ...
        BigDecimal newShares = amountBaseCurrency.multiply(getTotalShares())
                .divide(getTotalBaseReserves(), 18, BigDecimal.ROUND_DOWN);
        
        setTotalCarbonReserves(getTotalCarbonReserves().add(amountCarbonCredits));
        setTotalBaseReserves(getTotalBaseReserves().add(amountBaseCurrency));
        setTotalShares(getTotalShares().add(newShares));
    }

    public BigDecimal calculateExchangeRate() {
        return getTotalCarbonReserves().multiply(getExchangeRateMultiplier())
                .divide(getTotalBaseReserves(), 18, RoundingMode.DOWN);
    }
    ...
    public BigDecimal getBaseBalance(String address) {
        ...
        System.out.println("DEBUG: - Size: " + baseBalances.size());
        System.out.println("DEBUG: - Is null: " + (baseBalances == null));
        System.out.println("DEBUG: - Is empty: " + baseBalances.isEmpty());
        
        if (baseBalances != null) {
            baseBalances.forEach(b -> {
                System.out.println("DEBUG: - Balance entry:");
                System.out.println("DEBUG:   - Address: " + b.getAddress());
                System.out.println("DEBUG:   - Balance: " + b.getBalance());
                System.out.println("DEBUG:   - Exchange ID: " + (b.getExchange() != null ? b.getExchange().getId() : "null"));
            });
        }
        ...
        System.out.println("DEBUG: Final balance found: " + balance);
        return balance;
    }

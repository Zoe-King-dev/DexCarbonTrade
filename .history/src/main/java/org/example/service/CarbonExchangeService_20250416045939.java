...
public class CarbonExchangeService {
    ...
    public void addLiquidity(String userId, BigDecimal amountUsdc, BigDecimal maxSlippagePercentage) {
        ...
        // 獲取或創建 LiquidityPool
        final LiquidityPool pool = liquidityPoolRepository.findAll().stream()
                .findFirst()
                .orElseGet(() -> {
        ...
        final LiquidityPool savedPool = liquidityPoolRepository.save(pool);
        System.out.println("DEBUG: Pool updated with new total shares: " + savedPool.getTotalShares());

        // 創建或更新流動性提供者
        LiquidityProvider provider = liquidityProviderRepository.findById(new LiquidityProviderId(savedPool.getId(), userId))
                .orElseGet(() -> {
                    System.out.println("DEBUG: Creating new LiquidityProvider");
                    LiquidityProvider newProvider = new LiquidityProvider();
                    newProvider.setPoolId(savedPool.getId());
                    newProvider.setProviderAddress(userId);
                    newProvider.setPool(savedPool);
    ...
    public void removeLiquidity(String userId, BigDecimal amountUsdc, BigDecimal maxSlippagePercentage) {
        ...
        LiquidityProvider provider = liquidityProviderRepository.findById(new LiquidityProviderId(pool.getId(), userId))

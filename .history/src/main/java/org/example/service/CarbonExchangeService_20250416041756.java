...
public class CarbonExchangeService {
    ...
    public void addLiquidity(String userId, BigDecimal amountUsdc, BigDecimal maxSlippagePercentage) {
        ...
        System.out.println("DEBUG: Database query: SELECT balance FROM usdc_balances WHERE address = '" + userId + "'");
    ...
    private CarbonExchange getExchange() {
        System.out.println("DEBUG: Getting exchange with balances");
        CarbonExchange exchange = exchangeRepository.findById(1L)
                .orElseThrow(() -> new IllegalStateException("Exchange not found"));
        
        // 強制加載 baseBalances
        System.out.println("DEBUG （IMPORTANT）: Loading baseBalances for exchange");
        exchange.getBaseBalances().size(); // 觸發加載
        System.out.println("DEBUG （IMPORTANT）: Number of balances loaded: " + exchange.getBaseBalances().size());

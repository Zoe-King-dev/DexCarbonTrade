...
public class CarbonExchangeService {
    ...
    public void addLiquidity(String userId, BigDecimal amountUsdc, BigDecimal maxSlippagePercentage) {
        ...
        if (userUsdcBalance.compareTo(BigDecimal.ZERO) == 0) {
            // 如果用戶沒有餘額記錄，初始化為 100 USDC
            userUsdcBalance = new BigDecimal("100");
            exchange.setBaseBalance(userId, userUsdcBalance);
            exchangeRepository.save(exchange);
        }

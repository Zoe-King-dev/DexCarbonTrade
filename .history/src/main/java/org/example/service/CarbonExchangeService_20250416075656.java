...
public class CarbonExchangeService {
    ...
    public void swapBaseCurrencyForCarbonCredits(String trader, BigDecimal amountBaseCurrency, BigDecimal maxExchangeRate) {
        try {
            ...
            // 更新用戶 CCT 餘額
            CarbonCreditToken cct = carbonCreditTokenRepository.findByAddress(trader)
                .orElseGet(() -> {
                    CarbonCreditToken newCct = new CarbonCreditToken();
                    newCct.setAddress(trader);
                    newCct.setBalance(BigDecimal.ZERO);
                    return newCct;
                });
            cct.setBalance(cct.getBalance().add(amountTokens));
            ...
            logger.info("交換完成 - 新的 USDC 儲備: {}, 新的 CCT 儲備: {}, 用戶 CCT 餘額: {}", 
                pool.getUsdcReserves(), pool.getCctReserves(), cct.getBalance());
        } catch (Exception e) {
            logger.error("交換失敗", e);
            throw e;
        }

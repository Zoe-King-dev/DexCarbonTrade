...
public class CarbonExchangeService {
    ...
    private CarbonExchange getExchange() {
        System.out.println("DEBUG: Getting first available exchange");
        CarbonExchange exchange = exchangeRepository.findAll().stream()
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("No exchange found in database"));
        
        System.out.println("DEBUG: Found exchange with ID: " + exchange.getId());
        return exchange;
    }

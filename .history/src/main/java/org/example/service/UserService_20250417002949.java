...
public class UserService {
    ...
    protected void initializeUserBalances(User user) {
        ...
        try {
            ...
            if (pools.isEmpty()) {
                throw new IllegalStateException("No liquidity pool found. Please initialize the pool first.");
            }
            
            LiquidityPool pool = pools.get(0);
            if (pool.getExchangeId() == null) {
                throw new IllegalStateException("Liquidity pool exchange ID is not set.");
            }
